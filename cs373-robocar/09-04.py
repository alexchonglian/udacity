...
class robot:
    ...
    def move(self, motion, threshold=0.001): # Do not change the name of this function
        # ADD CODE HERE
        # update mutable state, not thread safe
        alpha, d = motion
        ...
        L, theta, x, y = self.length, self.orientation, self.x, self.y
        beta = d/L * tan(alpha)
        if abs(beta) > threshold:
            ...
            theta_prime = theta + beta
        self.x = x_prime
        self.y = y_prime
        self.orientation = theta_prime
