...
def bsuccessors(state):
    ...
    '<-' for there to here."""
    here, there, t = state
    zet = frozenset; lt = 'light'
    if 'light' in here:
        return { (here-zet([a,b,lt]), there|zet([a,b,lt]), t+max(a,b)): (a,b,'->')
            for a in here if a != lt
            for b in here if b != lt}
    elif 'light' in there:
        return { (here|zet([a,b,lt]), there-zet([a,b,lt]), t+max(a,b)): (a,b,'<-')
            for a in there if a != lt
            for b in there if b != lt}
