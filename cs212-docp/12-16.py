...
def bsuccessors2(state):
    ...
    here, there = state
    if 'light' in here:
        return dict(((here  - zet([a,b, lt]), there | zet([a, b, lt])),
                     (a, b, '->'))
                    for a in here if a is not lt
                    for b in here if b is not lt)
    else:
        return dict(((here  | zet([a,b, lt]), there - zet([a, b, lt])),
                     (a, b, '<-'))
                    for a in there if a is not lt
                    for b in there if b is not lt) 
