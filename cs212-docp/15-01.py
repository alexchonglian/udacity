...
def Pwin3(me, you, pending):
    ...
    if me + pending >= goal:
        ...
        p_roll_pig_out = 1/6.0*(1 - Pwin3(you, me+1, 0))
        p_roll_stay = 1/6.0*sum(Pwin3(me, you, pending+n) for n in (2,3,4,5,6))
        p_roll = p_roll_pig_out + p_roll_stay
        if pending == 0:
            return p_roll
        else:
            p_hold = 1 - Pwin3(you, me+pending, 0)
            return max(p_roll, p_hold)
