...
def create_map(pattern):
    ...
    outcome =  ['x' if i == '1' else '.' for i in bin(pattern)[2:].zfill(8)]
    return {i:j for i,j in zip(patterns, outcome)}
    
def cellular_automaton(row, pattern, gen):
    l = len(row)
    omap = create_map(pattern)
    for i in range(gen):
        row = row[-1] + row + row[0]
        row = ''.join([omap[row[i:i+3]] for i in range(l)])
    return row
