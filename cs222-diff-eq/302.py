...
def heuns_method(num_steps):
    ...
    ###Heun's Method
    for step in range(num_steps):
        xe = x[step] + h * v[step]#x[step+1]
        ve = v[step] + h * acceleration(x[step])#v[step+1]
        x[step + 1] = x[step] + h * (v[step]+ve)/2.0
        v[step + 1] = v[step] + h * (acceleration(x[step]) + acceleration(xe)) /2.0
