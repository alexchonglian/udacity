...
def involved0(courses, person):
    return {
        sem: [c for c,d in courses.items() 
            if ('teacher' in d and d['teacher'] == person) or ('assistant' in d and d['assistant'] == person)
            ] 
...
def involved(courses, person):
    inv = {}
    for sem, courses in courses.items():
        invcourses = [c for c,d in courses.items()
            if ('teacher' in d and d['teacher'] == person) 
            or ('assistant' in d and d['assistant'] == person)
        ]
        if invcourses:
            inv[sem] = invcourses
    return inv
