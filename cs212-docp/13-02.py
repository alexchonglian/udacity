    ...
    def successors(state):
        ...
        for i in range(lens):
            for j in range(lens):
                if i != j:
                    s = list(state)
                    diff = min(state[i], capacities[j]-state[j])
                    s[i] = state[i]-diff; s[j] = state[j]+diff
                    res[tuple(s)] = ('pour', i, j)
        return res
    return shortest_path_search(tuple([0]*lens), successors, lambda x: goal in x)
